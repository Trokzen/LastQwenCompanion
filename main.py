...
class ApplicationData(QObject):
    ...
    def updateAction(self, action_id: int, action_data: 'QVariant') -> bool:
        ...
        if self.database_manager:
            try:
                # Подготовка данных (фильтрация разрешенных полей)
                allowed_fields = ['algorithm_id', 'description', 'technical_text', 'start_offset', 'end_offset', 'contact_phones', 'report_materials', 'selected_organizations']
                prepared_data = {}
                for key, value in action_data.items():
                    if key in allowed_fields:
    ...
    # --- СЛОТЫ ДЛЯ РАБОТЫ С ОРГАНИЗАЦИЯМИ В ДЕЙСТВИЯХ ---

    @Slot(int, result='QVariant')
    def getOrganizationsForAction(self, action_id: int) -> 'QVariant':
        """
        Получить организации с их файлами для конкретного действия (шаблона).
        """
        print(f"Python: QML запросил организации для действия ID {action_id}.")
        ...
        if self.database_manager:
            try:
                organizations = self.database_manager.get_organizations_for_action(action_id)
                print(f"Python: Получено {len(organizations)} организаций для действия ID {action_id}.")
                return organizations
            except Exception as e:
                print(f"Python: Ошибка при получении организаций для действия {action_id}: {e}")
                import traceback
                traceback.print_exc()
                return []
        else:
        ...
        if self.database_manager:
            try:
                ...
                if success:
                    ...
                    print(f"Python: Не удалось обновить организации для действия ID {action_id}.")
                    return False
            except Exception as e:
                print(f"Python: Исключение при обновлении организаций для действия {action_id}: {type(e).__name__}: {e}")
                import traceback
                traceback.print_exc()
                return False
        else:
    ...
    @Slot(int, int, result=bool)
    def addOrganizationToActionExecution(self, action_execution_id: int, organization_id: int) -> bool:
        """Добавить организацию к мероприятию."""
        if self.database_manager:
            try:
                return self.database_manager.add_organization_to_action_execution(action_execution_id, organization_id)
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при добавлении организации к действию: {e}")
                return False
        return False

    @Slot(int, int, result=bool)
    def removeOrganizationFromActionExecution(self, action_execution_id: int, organization_id: int) -> bool:
        """Убрать организацию из мероприятия."""
        if self.database_manager:
            try:
                return self.database_manager.remove_organization_from_action_execution(action_execution_id, organization_id)
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при удалении организации из действия: {e}")
                return False
        return False

    # === СЛОТЫ ДЛЯ РАБОТЫ С ФАЙЛАМИ ОРГАНИЗАЦИЙ В МЕРОПРИЯТИЯХ ===

    @Slot(int, result='QVariant')
    def getOrganizationsWithFilesForActionExecution(self, action_execution_id: int):
        """Получить организации с их файлами для конкретного мероприятия."""
        if self.database_manager:
            try:
                result = self.database_manager.get_organizations_with_files_for_action_execution(action_execution_id)
                print(f"Python ApplicationData: getOrganizationsWithFilesForActionExecution вернул {len(result)} организаций")
                return result
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при получении организаций с файлами: {e}")
                return []
        return []

    @Slot(int, int, int, result=bool)
    def addOrganizationFileToActionExecution(self, action_execution_id: int, organization_id: int, reference_file_id: int) -> bool:
        """Привязать файл организации к мероприятию."""
        if self.database_manager:
            try:
                return self.database_manager.add_org_file_to_action_execution(action_execution_id, organization_id, reference_file_id)
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при привязке файла организации к действию: {e}")
                return False
        return False

    @Slot(int, int, int, result=bool)
    def removeOrganizationFileFromActionExecution(self, action_execution_id: int, organization_id: int, reference_file_id: int) -> bool:
        """Убрать файл организации из мероприятия."""
        if self.database_manager:
            try:
                return self.database_manager.remove_org_file_from_action_execution(action_execution_id, organization_id, reference_file_id)
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при удалении файла организации из действия: {e}")
                return False
        return False

    @Slot(int, result=bool)
    def clearOrganizationFilesForActionExecution(self, action_execution_id: int) -> bool:
        """Удалить все файлы организаций для мероприятия."""
        if self.database_manager:
            try:
                return self.database_manager.clear_org_files_for_action_execution(action_execution_id)
            except Exception as e:
                print(f"Python ApplicationData: Ошибка при очистке файлов организаций для действия: {e}")
                return False
        return False
