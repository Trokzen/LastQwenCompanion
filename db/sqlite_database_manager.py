...
class SQLiteDatabaseManager:
    ...
    def _init_db(self):
        ...
        if 'selected_organizations' not in existing_columns:
            try:
                ...
                logger.info("Миграция: добавлена колонка selected_organizations в actions.")
            except sqlite3.Error as e:
                logger.warning(f"Миграция: не удалось добавить selected_organizations: {e}")
    ...
    # === МЕТОДЫ ДЛЯ РАБОТЫ С ФАЙЛАМИ ОРГАНИЗАЦИЙ В МЕРОПРИЯТИЯХ ===

    def get_org_files_for_action_execution(self, action_execution_id: int) -> list:
        """
        Получить все файлы организаций для конкретного мероприятия.
        Возвращает список словарей с полями:
        id, action_execution_id, organization_id, organization_name, reference_file_id, 
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    aeof.reference_file_id,
                    orf.file_path,
                    orf.file_type,
                    aeof.created_at
                FROM action_execution_org_files aeof
                INNER JOIN organizations o ON aeof.organization_id = o.id
                INNER JOIN organization_reference_files orf ON aeof.reference_file_id = orf.id
            ...
            logger.error(f"SQLiteDatabaseManager: Ошибка при получении файлов организаций для действия: {e}")
            return []
        except Exception as e:
            logger.exception(f"SQLiteDatabaseManager: Неизвестная ошибка при получении файлов организаций для действия: {e}")
            return []

    def get_organizations_with_files_for_action_execution(self, action_execution_id: int) -> list:
        """
        Получить организации с их файлами для конкретного мероприятия.
        Возвращает список организаций, каждая из которых содержит список файлов.
        Формат: [{id, name, phone, contact_person, reference_files: [{id, file_path, file_type, created_at}]}]
        """
        try:
            conn = self._get_connection()
            conn.row_factory = sqlite3.Row
            ...
            # Получаем все организации, привязанные к действию
            cursor.execute("""
                SELECT 
                    o.id,
                    o.name,
            ...
            # Для каждой организации получаем её файлы для этого действия
            for org in organizations:
                cursor.execute("""
                    SELECT 
                        ...
                        orf.file_type,
                        orf.created_at
                    FROM organization_reference_files orf
                    INNER JOIN action_execution_org_files aeof ON orf.id = aeof.reference_file_id
                    WHERE aeof.action_execution_id = ? AND aeof.organization_id = ?
                    ORDER BY orf.file_type, orf.file_path
                """, (action_execution_id, org['id']))
            ...
            logger.error(f"SQLiteDatabaseManager: Ошибка при получении организаций с файлами для действия: {e}")
            return []
        except Exception as e:
            logger.exception(f"SQLiteDatabaseManager: Неизвестная ошибка при получении организаций с файлами для действия: {e}")
            return []

    def add_org_file_to_action_execution(self, action_execution_id: int, organization_id: int, reference_file_id: int) -> bool:
        """
        Привязать файл организации к мероприятию.
        """
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO action_execution_org_files (action_execution_id, organization_id, reference_file_id) 
                VALUES (?, ?, ?);
            """, (action_execution_id, organization_id, reference_file_id))
            conn.commit()
            cursor.close()
            conn.close()
            logger.info(f"SQLiteDatabaseManager: Файл {reference_file_id} организации {organization_id} добавлен к действию {action_execution_id}.")
            return True
        except sqlite3.IntegrityError as e:
            logger.warning(f"SQLiteDatabaseManager: Файл уже привязан к действию: {e}")
            return False
        except sqlite3.Error as e:
            logger.error(f"SQLiteDatabaseManager: Ошибка при привязке файла организации к действию: {e}")
            return False
        except Exception as e:
            logger.exception(f"SQLiteDatabaseManager: Неизвестная ошибка при привязке файла организации к действию: {e}")
            return False

    def remove_org_file_from_action_execution(self, action_execution_id: int, organization_id: int, reference_file_id: int) -> bool:
        """
        Убрать файл организации из мероприятия.
        """
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                DELETE FROM action_execution_org_files 
                WHERE action_execution_id = ? AND organization_id = ? AND reference_file_id = ?;
            """, (action_execution_id, organization_id, reference_file_id))
            conn.commit()
            affected_rows = cursor.rowcount
            cursor.close()
            conn.close()
            logger.info(f"SQLiteDatabaseManager: Удалено {affected_rows} файлов организации из действия ID {action_execution_id}.")
            return affected_rows > 0
        except sqlite3.Error as e:
            logger.error(f"SQLiteDatabaseManager: Ошибка при удалении файла организации из действия: {e}")
            return False
        except Exception as e:
            logger.exception(f"SQLiteDatabaseManager: Неизвестная ошибка при удалении файла организации из действия: {e}")
            return False

    def clear_org_files_for_action_execution(self, action_execution_id: int) -> bool:
        """
        Удалить все файлы организаций для конкретного мероприятия.
        """
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            ...
            cursor.close()
            conn.close()
            logger.info(f"SQLiteDatabaseManager: Очищены все файлы организаций для действия ID {action_execution_id}. Удалено: {affected_rows}")
            return True
        except sqlite3.Error as e:
            logger.error(f"SQLiteDatabaseManager: Ошибка при очистке файлов организаций для действия: {e}")
            return False
        except Exception as e:
            logger.exception(f"SQLiteDatabaseManager: Неизвестная ошибка при очистке файлов организаций для действия: {e}")
            return False

    # ========================================================================
    # МЕТОДЫ ДЛЯ РАБОТЫ С ОРГАНИЗАЦИЯМИ В ШАБЛОНАХ ДЕЙСТВИЙ (actions)
    # ========================================================================
