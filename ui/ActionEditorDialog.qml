...
Popup {
    ...
    // --- Свойства для организаций и файлов ---
    property var selectedOrganizations: []  // Массив организаций, выбранных для этого действия
    property var allOrganizationsList: []   // Все доступные организации из справочника

    // Для диалога выбора организации
    property int selectedOrgForAdding: -1   // ID организации, выбранной для добавления файлов
    property var selectedOrgFiles: []       // Файлы, выбранные для добавления к организации
    ...
    ColumnLayout {
        ...
        ScrollView {
            ...
            GridLayout {
                ...
                // === ОРГАНИЗАЦИИ И ФАЙЛЫ ДЛЯ МЕРОПРИЯТИЯ ===
                Label {
                    text: "Организации и справочные файлы:*"
                    Layout.alignment: Qt.AlignRight | Qt.AlignTop
                    Layout.topMargin: 10
                }
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Button {
                            ...
                            onClicked: {
                                // Загружаем все организации для выбора
                                actionEditorDialog.allOrganizationsList = appData.getAllOrganizations()
                                organizationSelectionDialog.open()
                            }
                        }

                        Label {
                            text: "(выберите организации и файлы для этого мероприятия)"
                            font.pixelSize: 11
                            color: "#666"
                        }
                    }
                    ...
                    ScrollView {
                        ...
                        ListView {
                            id: selectedOrgsList
                            model: actionEditorDialog.selectedOrganizations
                            spacing: 8

                            delegate: Rectangle {
                                width: ListView.view.width
                                height: orgFilesRepeater.height + 60
                                radius: 6
                                color: "#f8f9fa"
                                border.color: "#dee2e6"
                                border.width: 1
                                ...
                                ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Button {
                                            ...
                                            onClicked: {
                                                ...
                                                var orgs = actionEditorDialog.selectedOrganizations
                                                orgs.splice(index, 1)
                                                actionEditorDialog.selectedOrganizations = orgs
                                            }
                                        }
                                        ...
                                        Button {
                                            text: "📁 Выбрать файлы"
                                            onClicked: {
                                                actionEditorDialog.selectedOrgForAdding = modelData.id
                                                // Загружаем файлы этой организации
                                                var orgFiles = appData.getOrganizationReferenceFiles(modelData.id)
                                                fileSelectionDialogModel.clear()
                                                for (var i = 0; i < orgFiles.length; i++) {
                                                    fileSelectionDialogModel.append(orgFiles[i])
                                                }
                                                fileSelectionDialogLabel.text = "Выберите файлы для: " + modelData.name
                                    ...
                                    Text {
                                        text: "Файлы: " + (modelData.reference_files ? modelData.reference_files.length : 0) + " шт."
                                        font.pixelSize: 11
                                        color: "#666"
                                    }
                                        ...
                                        delegate: RowLayout {
                                            ...
                                            Button {
                                                ...
                                                onClicked: {
                                                    ...
                                                    var orgs = actionEditorDialog.selectedOrganizations
                                                    var files = orgs[index].reference_files
                                                    files.splice(modelDataIndex, 1)
                                                    orgs[index].reference_files = files
                                                    actionEditorDialog.selectedOrganizations = orgs
                                                }
                                            }
                                        }
                            ...
                            Label {
                                ...
                                text: "Нет выбранных организаций"
                                color: "#95a5a6"
                                font.pixelSize: 12
                                font.italic: true
                                visible: actionEditorDialog.selectedOrganizations.length === 0
                            }
                        }
                    }
                }
            ...
            Button {
                ...
                onClicked: {
                    ...
                    if (isEditMode) {
                        ...
                        // Если действие успешно добавлено, result содержит его ID
                    ...
                    if (result === true || (typeof result === 'number' && result > 0)) {
                        ...
                        // ✅ СОХРАНЯЕМ ВЫБРАННЫЕ ОРГАНИЗАЦИИ И ТОЛЬКО ВЫБРАННЫЕ ФАЙЛЫ
                        var actionOrganizations = [];
                        for (var i = 0; i < actionEditorDialog.selectedOrganizations.length; i++) {
                            var org = actionEditorDialog.selectedOrganizations[i];
                            var orgData = {
                                id: org.id,
                                selected_files: []
                            };
                            
                            // Собираем ТОЛЬКО ID выбранных файлов ДЛЯ ЭТОЙ ОРГАНИЗАЦИИ
                            if (org.reference_files && org.reference_files.length > 0) {
                                for (var j = 0; j < org.reference_files.length; j++) {
                                    orgData.selected_files.push(org.reference_files[j].id);
                                }
                            }
                            
                            actionOrganizations.push(orgData);
                        }
                        
                        console.log("QML ActionEditorDialog: Сохраняем организации и выбранные файлы:", JSON.stringify(actionOrganizations));
                        appData.updateActionOrganizations(actionId, actionOrganizations);
    ...
    function resetForAdd(algorithmId) {
        ...
        actionEditorDialog.selectedOrganizations = []; // Сбрасываем организации
        errorMessageLabel.text = "";
    }
    ...
    function loadDataForEdit(actionData) {
        ...
        // Загружаем организации и файлы для этого действия (шаблона)
        var orgs = appData.getOrganizationsForAction(actionData.id);
        if (orgs && orgs.length > 0) {
            actionEditorDialog.selectedOrganizations = orgs;
            console.log("QML ActionEditorDialog: Загружено", orgs.length, "организаций для действия");
        } else {
            actionEditorDialog.selectedOrganizations = [];
            console.log("QML ActionEditorDialog: Организации для действия не найдены");
        }
    }
    ...
    */
    ...
    Dialog {
        id: organizationSelectionDialog
        ...
        ColumnLayout {
            ...
            ScrollView {
                ...
                ListView {
                    id: orgSelectionList
                    model: actionEditorDialog.allOrganizationsList
                    spacing: 4
                    ...
                    delegate: Rectangle {
                        ...
                        height: 40
                        radius: 4
                        color: mouseAreaOrg.containsMouse ? "#e3f2fd" : (index % 2 ? "#f9f9f9" : "#ffffff")
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (alreadyAdded) {
                                    showMessage("Организация \"" + modelData.name + "\" уже добавлена.")
                                } else {
                                    // Добавляем организацию с пустым списком файлов
                                    var newOrg = {
                                        id: modelData.id,
                                        name: modelData.name,
                                    ...
                                    orgs.push(newOrg)
                                    actionEditorDialog.selectedOrganizations = orgs
                                    organizationSelectionDialog.close()
                                }
                            }
                        }
                    }
                    ...
                    Label {
                        anchors.centerIn: parent
                        text: "Нет доступных организаций"
                        color: "#95a5a6"
                        font.pixelSize: 12
                        font.italic: true
                        visible: actionEditorDialog.allOrganizationsList.length === 0
                    }
                }
            }
        }
        ...
        footer: DialogButtonBox {
            Button {
                text: "Отмена"
                onClicked: organizationSelectionDialog.close()
            }
        }
    }

    // === ДИАЛОГ ВЫБОРА ФАЙЛОВ ОРГАНИЗАЦИИ ===
    ListModel {
        id: fileSelectionDialogModel
    }
    ...
    Dialog {
        ...
        ColumnLayout {
            ...
            Label {
                id: fileSelectionDialogLabel
                text: "Выберите файлы для организации:"
                font.bold: true
            }
            ...
            ScrollView {
                ...
                ListView {
                    id: fileSelectionList
                    model: fileSelectionDialogModel
                    spacing: 4
                    ...
                    delegate: Rectangle {
                        ...
                        height: 45
                        radius: 4
                        color: mouseAreaFile.containsMouse ? "#e3f2fd" : (index % 2 ? "#f9f9f9" : "#ffffff")
                        ...
                        RowLayout {
                            ...
                            // Тип файла
                            Text {
                                text: (model.file_type || "other").toUpperCase()
                                font.pixelSize: 10
                                color: "#666"
                            }
                        }
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                var orgs = actionEditorDialog.selectedOrganizations
                                var targetOrgIndex = -1
                                for (var i = 0; i < orgs.length; i++) {
                                    if (orgs[i].id === actionEditorDialog.selectedOrgForAdding) {
                                        targetOrgIndex = i
                                        break
                                    }
                                }
                                ...
                                if (targetOrgIndex >= 0) {
                                    var files = orgs[targetOrgIndex].reference_files
                                    var alreadyAdded = false
                                    for (var j = 0; j < files.length; j++) {
                                        if (files[j].id === model.id) {
                                            alreadyAdded = true
                                            break
                                        }
                                    }
                                    ...
                                    if (alreadyAdded) {
                                        ...
                                        orgs[targetOrgIndex].reference_files = files
                                        actionEditorDialog.selectedOrganizations = orgs
                                    }
                                }
                            }
                    ...
                    Label {
                        anchors.centerIn: parent
                        text: "Нет файлов у этой организации"
                        color: "#95a5a6"
                        font.pixelSize: 12
                        font.italic: true
                        visible: fileSelectionDialogModel.count === 0
                    }
                }
            }
        }
        ...
        footer: DialogButtonBox {
            Button {
                text: "Закрыть"
                onClicked: fileSelectionDialog.close()
            }
        }
    }
